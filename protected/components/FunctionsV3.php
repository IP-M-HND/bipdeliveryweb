...
class FunctionsV3
{
    ...
    public static function getMenu($class="menu")
    {
     ...
     // $top_menu[]=array('tag'=>"serviciodepaqueteria",'label'=>''.Yii::t("default","Servicio de Paqueteria"),
			// 		       'url'=> array('/store/paqueteria'));

			// $top_menu[]=array('tag'=>"Clinpays",'label'=>''.Yii::t("default","Clinpays"),
      //           'url'=>'https://clinpays.com/');

			$top_menu[]=array('tag'=>"Llama",'label'=>''.Yii::t("default","<div class='hidden-xs hidden-sm'> <h2  style='background: #00ccbc;padding: 15px;border-radius: 80px;'>Llama al 2512-4060!<h2/></div>"),
									'url'=>'');
    ...
    public static function merchantOpenTag($merchant_id='')
    {
        ...
        if ($is_merchant_open){
        	$tag='<span class="label label-success">'.t("Abierto").'</span>';
        } else {
        	if ($merchant_preorder){
        		$tag='<span class="label label-info">'.t("Pre-Orden").'</span>';
        	} else {
        		$tag='<span class="label label-danger">'.t("Cerrado").'</span>';
        	}
        }
        return $tag;
    }
